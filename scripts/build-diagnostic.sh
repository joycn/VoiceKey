#!/usr/bin/env bash
# Build only. Maintenance flashing instructions are in docs/maintenance.md.
set -eo pipefail
repo_root="$(cd "$(dirname "$0")/.." && pwd)"
if [[ $# != 0 ]]; then echo 'This wrapper builds only; no flash arguments accepted.' >&2; exit 2; fi
...
cd "$repo_root/firmware/diagnostic"
idf.py build
python "$repo_root/scripts/validate-diagnostic.py"
